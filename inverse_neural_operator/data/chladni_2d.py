...
class ChladniDataset(Dataset):
    ...
    def __init__(self, dataset, device="cpu"):
        ...
        self.X = torch.tensor(
            dataset["X"], device=device, dtype=torch.float32
        )  # Input coordinates
        self.u = torch.tensor(
            dataset["u"], device=device, dtype=torch.float32
        )  # Input function (forces)
        self.Y = torch.tensor(
            dataset["Y"], device=device, dtype=torch.float32
        )  # Output coordinates
        self.s = torch.tensor(
            dataset["s"], device=device, dtype=torch.float32
        )  # Output function (displacements)
...
def generate_chladni_data():
    ...
    centerFactor = np.cos(mu_vals[:, None] * (L / 2)) * np.cos(
        lam_vals[None, :] * (M / 2)
    )
    ...
    setONet_data = {
        ...
        "u": np.zeros(
            (N_total, n_points), dtype=np.float32
        ),  # Input function (S forces)
        "Y": np.zeros((N_total, n_points, 2), dtype=np.float32),  # Output coordinates
        "s": np.zeros(
            (N_total, n_points), dtype=np.float32
        ),  # Output function (Z displacements)
    }
    ...
    np.savez_compressed(
        "data/ChladniData_normalization.npz",
        u_mean=u_mean,
        u_std=u_std,
        s_mean=s_mean,
        s_std=s_std,
