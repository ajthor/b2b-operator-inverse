...
def visualize_ifno_results(
    model_path, n_samples=3, save_dir="results/ifno_plots_parametric_heat/"
):
    ...
    modes1 = (
        inferred_cfg.get("modes1") if inferred_cfg and "modes1" in inferred_cfg else 8
    )
    modes2 = (
        inferred_cfg.get("modes2") if inferred_cfg and "modes2" in inferred_cfg else 8
    )
    width = (
        inferred_cfg.get("width") if inferred_cfg and "width" in inferred_cfg else 24
    )
    n_layers = (
        inferred_cfg.get("n_layers")
        if inferred_cfg and "n_layers" in inferred_cfg
        else 3
    )
    vae_latent_dim = (
        inferred_cfg.get("vae_latent_dim")
        if inferred_cfg and "vae_latent_dim" in inferred_cfg
        else 8
    )
...
def plot_sample(model, sample, sample_idx, save_dir, dataset_info):
    ...
    im0 = axes_inv[0].imshow(
        s_true_2d, origin="lower", extent=[0, 1, 0, 1], cmap="viridis"
    )
    ...
    im1 = axes_inv[1].imshow(
        u_true_2d, origin="lower", extent=[0, 1, 0, 1], cmap="viridis"
    )
    ...
    im2 = axes_inv[2].imshow(
        u_pred_2d, origin="lower", extent=[0, 1, 0, 1], cmap="viridis"
    )
    ...
    im5 = axes_fwd[2].imshow(
        s_pred_2d, origin="lower", extent=[0, 1, 0, 1], cmap="viridis"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Visualize IFNO results on Parametric Heat 2D dataset"
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="results/ifno_plots_parametric_heat/",
    ...
    if not os.path.exists(args.model_path):
        ...
        print(
            "Please train the model first using: python train_ifno_standalone.py --dataset parametric_heat"
        )
        exit(1)
