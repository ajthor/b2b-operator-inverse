...
- Left: Predicted permeability functions a(x) (1D line plots)
- Right: Re-simulated pressure fields u(x) (1D line plots)

To run: cd /workspaces/b2b-operator-inverse && python -m inverse_neural_operator.plots.plot_darcy_nine_publication
"""
...
def _create_unified_figure():
    """Create figure with unified gridspec for Darcy 1D visualization.

    Returns:
        fig: Figure object
        gs: GridSpec object
        axes_left: List of axes for left grid (permeability plots)
        axes_right: List of axes for right grid (pressure plots)
    """
    ...
    ax_left_parent.set_ylabel("Permeability $a(x)$", labelpad=-8)
    ax_left_parent.set_title("Predicted Permeability (Normalized)")
    ...
    ax_right_parent.set_ylabel("Pressure $u(x)$", labelpad=-8)
    ax_right_parent.set_title("Darcy 1D Re-simulated Pressure (Normalized)")

    # Create subplot axes for left grid (permeability plots)
    axes_left = []
    for i in range(3):
        for j in range(3):
    ...
    # Create subplot axes for right grid (pressure plots)
    axes_right = []
    for i in range(3):
        for j in range(3):
...
def collect_darcy_predictions(
    ...
    """Collect predictions specifically for Darcy 1D problem.
    ...
    # Note: In Darcy code, u is input (permeability), s is output (pressure)
        ...
        with torch.no_grad():
            ...
            if model_name in SAMPLING_MODELS:
                ...
                if alpha_samples is not None:
                    # Reconstruct permeability from sampled alpha coefficients
...
def plot_comparison(sample_idx, models_to_plot, predictions, meta, save_dir):
    """Create publication-quality comparison plot for Darcy 1D.
    ...
    x_coords = meta["x"]  # [N, 1]
    u_true = meta["u_true"]  # Permeability [N]
    y_coords = meta["y"]  # [M, 1]
    s_true = meta["s_true"]  # Pressure [M]
    ...
    # Left side: Permeability curves (top 5 models + ground truth)
    ...
    # Right side: Pressure curves (top 5 models + ground truth)
    ...
    base = os.path.join(save_dir, f"darcy_norm_sample_{sample_idx}_publication")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create publication-quality Normalized Darcy 1D plots."
    )
    ...
    for idx in sample_indices:
        ...
        predictions, meta = collect_darcy_predictions(
            test_dataset[idx],
            models_to_plot,
            models_dict,
            input_enc,
