...
MODELS = ['b2b_linear', 'b2b_nonlinear',
          'variational_autoencoder', 'invertible_network', 'realnvp']
...
def plot_burgers_sample(model, evaluate_fn, input_function_encoder, output_function_encoder,
    ...
    with torch.no_grad():
        point = (X.unsqueeze(0), u_true.unsqueeze(
            0), Y.unsqueeze(0), s.unsqueeze(0))
        u_pred = evaluate_fn(
    ...
    if grid_size * grid_size != n_points:
        ...
        axes[1].plot(x_coords, u_pred_np, 'r--',
                     label='Predicted Input', alpha=0.7)
        ...
        im1 = axes[0].imshow(s_2d, cmap='viridis',
        ...
        im2 = axes[1].imshow(error_2d, cmap='Reds',
                             extent=extent, origin='lower')
        axes[1].set_title(
            'Input Prediction Error |u_pred - u_true|', fontsize=12)
        axes[1].set_xlabel('x')
        axes[1].set_ylabel('y')
    ...
    if save_dir:
        ...
        save_path = os.path.join(
            save_dir, f'{model_name}_sample_{sample_idx}.png')
...
def plot_model_results(model_name, log_dir, results_dir, test_dataset, dataset_info, n_samples=3):
    ...
    params = torch.load(os.path.join(
        model_log_dir, "params.pth"), weights_only=False)
...
parser = argparse.ArgumentParser(
    description="Plot Burgers 1D results for all models.")
...
temp_params = torch.load(os.path.join(
    temp_log_dir, "params.pth"), weights_only=False)
