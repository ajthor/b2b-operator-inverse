"""
Plot probabilistic results for the Darcy 1D dataset with multiple realizations.
...
To run: cd /workspaces/b2b-operator-inverse && python -m inverse_neural_operator.plots.plot_darcy_probabilistic
"""
...
def plot_darcy_probabilistic_sample(
    model,
    evaluate_fn,
    ...
    """
    Plot a single Darcy sample with probabilistic realizations.
    """
    model.eval()
    forward_model.eval()
...
def plot_multiple_samples(
    ...
    for i, idx in enumerate(test_indices):
        ...
        plot_darcy_probabilistic_sample(
            model,
            evaluate_fn,
            input_function_encoder,
            output_function_encoder,
...
parser = argparse.ArgumentParser(description="Plot Darcy 1D probabilistic results.")
parser.add_argument(
    "--log_dir",
    type=str,
...
parser.add_argument(
    "--results_dir",
    type=str,
    default="results/darcy_probabilistic_plots",
...
print(f"SUCCESS: Plotted {model_name} probabilistic results, {args.n_samples} samples with {args.n_realizations} realizations each")