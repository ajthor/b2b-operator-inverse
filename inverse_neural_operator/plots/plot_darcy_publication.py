...
def _collect_predictions(
    ...
    """Collect multiple samples of predictions and outputs for one test sample."""
    ...
    for model_name in models_to_plot:
        ...
        with torch.no_grad():
            for _ in range(N_SAMPLES_PER_MODEL):
                # Many evaluate() helpers return (u_pred, latent) where the
                # second element is a latent/code (alpha) rather than a
                # direct re-simulation. If an output_transform is provided
                # we use it to convert the latent into a re-simulated output
                # field s(y). Otherwise, if the evaluate function already
                # returns an s-like tensor we accept it.
                eval_out = evaluate_fn(
                    model, point, input_function_encoder, output_function_encoder
                )

                # Normalize outputs to a pair (u_pred, maybe_latent_or_s)
                ...
                # If there's no second value, nothing to do for outputs.
                if second is None:
                    continue

                # If the caller provided an output_transform (forward model +
                # output encoder), prefer that to convert a latent into a
                # re-simulated s-field.
                if output_transform is not None:
                    try:
                        ...
                        # Fall back to trying to interpret `second` directly
                        pass

                # Fall back: if `second` already looks like an s-field (same
                # length as the true s), accept it.
                try:
                    s_resim_np = second.squeeze(0).detach().cpu().numpy().flatten()
                    if s_resim_np.size == meta["s_true"].size:
                        output_samples.append(s_resim_np)
                except Exception:
                    # Can't interpret `second` as s; skip.
                    continue
...
def _plot_single_row(
    ...
    ax_right_parent.set_ylabel(r"$s(y)$", labelpad=-8)
    ax_right_parent.set_title("Output Predictions")
    ...
    for idx, model_name in enumerate(models_to_plot):
        ...
        ax_in.tick_params(labelsize=6, length=2, width=0.5)
        ax_in.set_ylabel("")
        ax_in.set_xlabel("")
        # keep tick locations so grid lines can be drawn; hide tick marks
        # and labels to preserve a clean publication look
        ax_in.tick_params(labelbottom=False, labelleft=False, length=0)
        ...
        if res_outputs.size:
            ...
            # keep tick locations so grid lines can be drawn; hide tick marks
            # and labels to preserve a clean publication look
            ax_out.tick_params(labelbottom=False, labelleft=False, length=0)
    ...
    if save_dir:
        ...
        base = os.path.join(
            save_dir, f"darcy_comparison_sample_{sample_idx}_publication"
        )
        fig.savefig(
...
def plot_all_models_comparison(
    ...
    """Create the single-row publication figure for Darcy."""
    ...
    # Attempt to load a forward model to convert latent codes into
    # re-simulated outputs; if unavailable, we'll accept direct s outputs
    # returned by evaluate() where possible.
    forward_model = _load_forward_model(log_dir, seed)
    output_transform = _make_darcy_output_transform(
...
def _make_darcy_output_transform(forward_model, output_function_encoder):
    """Create a callable that maps latent codes to re-simulated s-fields.

    This mirrors the Burgers script approach: forward_model maps latent ->
    beta coefficients, and output_function_encoder maps (Y, beta) -> s(y).
    """
...
def _load_forward_model(log_dir: str, seed: int):
    """Load a forward model (if available) for Darcy re-simulations."""
