...
class CinnAffineProbabilistic(torch.nn.Module):
    def __init__(self, coupling_layers):
        super(CinnAffineProbabilistic, self).__init__()
...
def create_model(
    ...
    Returns:
        CinnAffineProbabilistic instance
    """
    coupling_layers = []
    ...
    return CinnAffineProbabilistic(coupling_layers=coupling_layers)
...
def loss_function(model, batch, input_function_encoder, output_function_encoder):
    ...
    -log p(alpha | beta) = -log p(z) - log |det J|
                         = 0.5 * ||z||^2 + 0.5 * log(2π) * dim(z) - log |det J|

    For affine coupling, we need both the Gaussian prior term and the change-of-variables term.
    """
    X, u, Y, s = batch
    ...
    z, log_det_J = model.forward(alpha_gt, beta_gt)
...
def resimulation_loss(
    ...
    """
    Compute re-simulation loss for probabilistic affine cINN model.
