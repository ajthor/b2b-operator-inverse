...
class MixtureDensityNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        # Component means (μ)
        self.mu_head = torch.nn.Linear(final_hidden_size, n_components * output_size, bias=bias)
        ...
        self.precision_head = torch.nn.Linear(final_hidden_size, n_components * n_precision_params, bias=bias)
    ...
    def forward(self, y):
        ...
        precision_flat = self.precision_head(x)  # (batch_size, n_components * n_precision_params)
        n_precision_params = self.output_size * (self.output_size + 1) // 2
        precision_params = precision_flat.view(batch_size, self.n_components, n_precision_params)
    ...
    def _build_precision_matrices(self, precision_params):
        ...
        precision_matrices = torch.zeros(
            batch_size, n_components, self.output_size, self.output_size,
            device=precision_params.device, dtype=precision_params.dtype
        )
        ...
        for k in range(n_components):
            ...
            L = torch.zeros(batch_size, self.output_size, self.output_size,
                          device=precision_params.device, dtype=precision_params.dtype)
            ...
            L[:, diag_indices, diag_indices] = F.softplus(L[:, diag_indices, diag_indices]) + 1e-3
    ...
    def inverse(self, beta):
        ...
        with torch.no_grad():
            ...
            selected_mu = mu[torch.arange(batch_size), component_indices]  # (batch_size, output_size)
            selected_precision = precision_matrices[torch.arange(batch_size), component_indices]  # (batch_size, output_size, output_size)
            
            # Convert precision to covariance for sampling
            selected_covariance = torch.inverse(selected_precision)
            
            # Sample from multivariate normal
            dist = torch.distributions.MultivariateNormal(selected_mu, selected_covariance)
            alpha = dist.sample()
    ...
    def log_prob(self, alpha, beta):
        ...
        # Use log_softmax for mixture weights (more numerically stable)
        pi_logits = self.pi_head(self._get_hidden_features(beta))
        log_pi = F.log_softmax(pi_logits, dim=-1)  # (batch_size, n_components)
        
        # Compute log probabilities for each component
        log_probs_components = torch.zeros(batch_size, self.n_components, device=alpha.device)
        
        for k in range(self.n_components):
            # Extract component parameters
            mu_k = mu[:, k, :]  # (batch_size, output_size)
            precision_k = precision_matrices[:, k, :, :]  # (batch_size, output_size, output_size)
            
            # Compute quadratic term: (x - μ)^T Σ^{-1} (x - μ)
            diff = alpha - mu_k  # (batch_size, output_size)
            quadratic = torch.sum(diff.unsqueeze(-2) @ precision_k @ diff.unsqueeze(-1), dim=(-2, -1))
            quadratic = quadratic.squeeze(-1)  # (batch_size,)
            
            # Compute log determinant of precision matrix with numerical safeguards
            # Add small regularization to diagonal for numerical stability
            regularized_precision_k = precision_k + 1e-6 * torch.eye(self.output_size, device=precision_k.device, dtype=precision_k.dtype).unsqueeze(0)
            log_det_precision = torch.logdet(regularized_precision_k)  # (batch_size,)
            ...
            normalization_constant = -0.5 * self.output_size * torch.log(torch.tensor(2 * torch.pi, device=alpha.device, dtype=alpha.dtype))
            log_prob_k = -0.5 * quadratic + 0.5 * log_det_precision + normalization_constant
...
def loss_function(
    model, batch, input_function_encoder, output_function_encoder, forward_model=None, lambda_forward=0.0
):
    """
...
def evaluate(model, point, input_function_encoder, output_function_encoder):
    ...
    with torch.no_grad():
        ...
        alpha_pred = model.inverse(beta)
        pred = input_function_encoder(Y, alpha_pred)  # Use Y instead of X since we're predicting input function
