...
forward_operator = torch.linalg.lstsq(source_coefficients, target_coefficients).solution
...
output_coefficients = torch.einsum("bk,kl->bl", input_coefficients, forward_operator)
...
# Inverse operator
inverse_operator = torch.linalg.inv(forward_operator)
...
output_coefficients = output_function_encoder.compute_coefficients(Y, Tf)
input_coefficients = torch.einsum("bk,kl->bl", output_coefficients, inverse_operator)

f_pred = input_function_encoder(X, input_coefficients)
...
ax.plot(Y, Tf, label="Original")
ax.scatter(Y, Tf, label="Data", color="red")

ax.plot(X, f, label="True")
ax.plot(X, f_pred, label="Prediction")
