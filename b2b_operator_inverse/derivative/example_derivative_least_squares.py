...
true_output_coefficients = output_function_encoder.compute_coefficients(Y, Tf)
...
alpha = torch.nn.Parameter(torch.randn(1, 8))
...
with tqdm.tqdm(range(1000)) as tqdm_bar:
    for epoch in tqdm_bar:
        ...
        mse_loss = torch.nn.functional.mse_loss(
            torch.einsum("bk,kl->bl", alpha, forward_operator), true_output_coefficients 
        )
        reg_loss = torch.norm(alpha, p=2)  # L2 regularization
        loss = mse_loss + 1e-3 * reg_loss  # Adjust the regularization weight as needed
        loss.backward(retain_graph=True)
        optimizer.step()

        tqdm_bar.set_postfix_str(f"Loss {loss.item():.3e}")

alpha_pred = alpha.detach()
